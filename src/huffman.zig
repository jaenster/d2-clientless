//! D2GS server->client packet huffman decompression (the payload of 0xAE blobs).
//! Tables + algorithm ported verbatim from jaenster/D2PacketBased (Tools/Compression.cpp).
const std = @import("std");

const CharIndex = [256]u16{
    0x0247, 0x0236, 0x0225, 0x0214, 0x0203, 0x01f2, 0x01e1, 0x01d0,
    0x01bf, 0x01ae, 0x019d, 0x018c, 0x017b, 0x016a, 0x0161, 0x0158,
    0x014f, 0x0146, 0x013d, 0x0134, 0x012b, 0x0122, 0x0119, 0x0110,
    0x0107, 0x00fe, 0x00f5, 0x00ec, 0x00e3, 0x00da, 0x00d1, 0x00c8,
    0x00bf, 0x00b6, 0x00ad, 0x00a8, 0x00a3, 0x009e, 0x0099, 0x0094,
    0x008f, 0x008a, 0x0085, 0x0080, 0x007b, 0x0076, 0x0071, 0x006c,
    0x0069, 0x0066, 0x0063, 0x0060, 0x005d, 0x005a, 0x0057, 0x0054,
    0x0051, 0x004e, 0x004b, 0x0048, 0x0045, 0x0042, 0x003f, 0x003f,
    0x003c, 0x003c, 0x0039, 0x0039, 0x0036, 0x0036, 0x0033, 0x0033,
    0x0030, 0x0030, 0x002d, 0x002d, 0x002a, 0x002a, 0x0027, 0x0027,
    0x0024, 0x0024, 0x0021, 0x0021, 0x001e, 0x001e, 0x001b, 0x001b,
    0x0018, 0x0018, 0x0015, 0x0015, 0x0012, 0x0012, 0x0012, 0x0012,
    0x000f, 0x000f, 0x000f, 0x000f, 0x000c, 0x000c, 0x000c, 0x000c,
    0x0009, 0x0009, 0x0009, 0x0009, 0x0006, 0x0006, 0x0006, 0x0006,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003, 0x0003,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
};

const CharTable = [600]u8{
    ,
};

const BitMasks = [16]u32{ 0x0000, 0x0001, 0x0003, 0x0007, 0x000F, 0x001F, 0x003F, 0x007F, 0x00FF, 0x01FF, 0x03FF, 0x07FF, 0x0FFF, 0x1FFF, 0x3FFF, 0x7FFF };

/// Decompress D2GS huffman `in` into `out`; returns the number of decompressed bytes,
/// or null on overflow. Mirrors DecompressDataContent (MSB-first 32-bit bit buffer).
pub fn decompress(in: []const u8, out: []u8) ?usize {
    var b: u32 = 0;
    var size: usize = in.len;
    var inptr: usize = 0;
    var maxcnt: usize = out.len;
    var outptr: usize = 0;
    var cnt: u32 = 0x20;
    while (true) {
        if (cnt >= 8) {
            while (size > 0 and cnt >= 8) {
                cnt -= 8;
                size -= 1;
                b |= @as(u32, in[inptr]) << @intCast(cnt);
                inptr += 1;
            }
        }
        const index: usize = CharIndex[b >> 0x18];
        const a: u32 = CharTable[index];
        const d: u32 = (b >> @intCast(0x18 - a)) & BitMasks[a];
        const c: u32 = CharTable[index + 2 * d + 2];
        cnt += c;
        if (cnt > 0x20) return out.len - maxcnt;
        if (maxcnt == 0) return null;
        maxcnt -= 1;
        out[outptr] = CharTable[index + 2 * d + 1];
        outptr += 1;
        b <<= @intCast(c & 0xFF);
    }
}
